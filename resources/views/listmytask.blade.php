            ...
            <div class="container" style="padding-bottom: 30px">
                                ...
                                <div class="modal-content">
                                    <form action="/done/store" method="post">
                                        ...
                                        <div class="modal-footer">
                                            @if (Auth::check())
                                                <input style="display:none;" type="text" class="hidden"
                                                    name="userid" required="required" id="userid"
                                                    value="{{ Auth::user()->id }}">
                                            @endif
                ...
                <div class="row">
                    <div class="account-profile1">
                        ...
                        <li><a href="/addmytasktodo" style="color: black">
                                <i class="fa-solid fa-circle-plus"></i>
                            </a></li>
                    </div>
                </div>
                <div class="row">
                    @foreach ($tasktodo as $todo)
                        <div class="menu-label"
                            style="list-style-type: none; border-radius: 37px; background-color: rgba(255,255,255,1);">
                            <div class="col-sm-10">
                                <li><a href="" style="color:black">{{ $todo->nametodo }}</a></li>
                            </div>
                            <div class="col-sm-2" style="display:flex">
                                <a href=""
                                    style="color:{{ round((strtotime($todo->deadlinetodo) - time()) / (60 * 60 * 24)) >= 7 ? '#12D248' : '#FF0000' }}">{{ round((strtotime($todo->deadlinetodo) - time()) / (60 * 60 * 24)) }}
                                    day</a>
                                <a href="#" style="color:black; margin-left:30px"><i class="fa-solid fa-play"
                                        data-toggle="modal" data-target="#myModal2_{{ $todo->id }}"></i></a>
                            </div>
                        </div>
                        <div class="modal fade" id="myModal2_{{ $todo->id }}">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <form action="/done/store" method="post">
                                        @csrf
                                        @php
                                            $tasktodos = DB::table('tasktodos')
                                                ->select('tasktodos.*')
                                                ->where('tasktodos.id', '=', $todo->id)
                                                ->get();
                                        @endphp
                                        @foreach ($tasktodos as $todos)
                                            <!-- Modal Header -->
                                            <div class="modal-header">
                                                <h4 class="modal-title" id="taskname" name="taskname">
                                                    {{ $todos->nametodo }}</h4>
                                            ...
                                            <div class="modal-body">
                                                <p id="taskdescription" name="taskdescription">
                                                    {{ $todos->descriptiontodo }}</p>
                                                <input type="hidden" name="taskname"
                                                    value="{{ $todos->nametodo }}">
                                                <input type="hidden" name="taskdescription"
                                                    value="{{ $todos->descriptiontodo }}">
                                            </div>
                                        @endforeach
                                        <!-- Modal Footer -->
                                        <div class="modal-footer">
